...
class job_selection extends CI_Model{
    ...
    function run_selection($opportunity_id,$intake_value,$weights, $percentage_value){
      ...
      //}
        ...
        foreach ($query1->result() as $row1)
        {
            ...
            $query2 = $this->db->get(); //get data related to each applicant

            foreach ($query2->result() as $row2)
            {
                $value1 = ($row2->certificate)*$weights['certificate'] +   //calculate total value by multiplying
                    ($row2->diploma)*$weights['diploma'] +                //each applicant's count of qualification
                    ($row2->bachelor)*$weights['bachelors'] +            //with the weight specified.
                    ($row2->masters)*$weights['masters'] +
                    ($row2->phd)*$weights['phd'] +
                    ($row2->job_full)*$weights['job_full'] +
                    ($row2->job_part)*$weights['job_part'] +
                    ($row2->intern_full)*$weights['intern_full'] +
                    ($row2->project)*$weights['project'] +
                    ($row2->hackathon)*$weights['hackathon'] +
                    ($row2->keywork)*$weights['keywork'] +
                    ($row2->society)*$weights['society'] +
                    ($row2->volunteering)*$weights['volunteering'] +
                    ($row2->sport)*$weights['sport'] +
                    ($row2->aesthetic)*$weights['aesthetic'] +
                    ($row2->blogging)*$weights['blogging'] ;
